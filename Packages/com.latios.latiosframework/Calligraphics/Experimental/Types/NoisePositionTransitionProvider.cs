...
namespace Latios.Calligraphics
{
    public struct NoisePositionTransitionProvider : ITransitionProvider
    {
        ...
        public unsafe void SetValue(ref DynamicBuffer<RenderGlyph> renderGlyphs, TextAnimationTransition transition, GlyphMapper glyphMapper, int startIndex, int endIndex, float normalizedTime)
        {
            ...
            for (int i = startIndex; i <= math.min(endIndex, renderGlyphs.Length - 1); i++)
            {
                ...
                switch (transition.scope)
                {
                    case TextScope.Glyph:
                        ...
                        break;
                    case TextScope.Word:
                        SetPositionValue(ref renderGlyph, values[transition.endIndex - glyphMapper.GetGlyphWordIndex(i)]);
                        break;
                    case TextScope.Line:
                        SetPositionValue(ref renderGlyph, values[transition.endIndex - glyphMapper.GetGlyphLineIndex(i)]);
                        break;
                    default:
